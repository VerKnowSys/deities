...
pub enum Mortal {
    ...
    NotificationConfigFailure { service: Service, cause: Error },
...
impl Display for Mortal {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f,
               "Mortal: {}",
               match self {
                   &Mortal::OkAllChecks { ref service, ref amount } => {
                       format!("Ok: {} successfully passed all: {} checks!",
                               service,
                               amount)
                   }
                   ...
                   &Mortal::OkPidAlive { ref service, ref pid } => {
                       format!("Ok: Alive pid: {} of service: {}", pid, service)
                   }
                   &Mortal::OkPidInterrupted { ref service, ref pid } => {
                       format!("Ok: Interrupted pid: {} of service: {}", pid, service)
                   }
                   &Mortal::OkPidAlreadyInterrupted { ref service, ref pid } => {
                       format!("Ok: Already interrupted pid: {} of service: {}",
                               pid,
                               service)
                   }
                   ...
                   &Mortal::CheckPidDead { ref service, ref pid } => {
                       format!("Found dead pid: {} of {}!", service, pid)
                   }

                   &Mortal::RawLoadFailure { ref file_name, ref cause } => {
                       format!("Can't load a raw file: {}. Reason: {}!", file_name, cause)
                   }
                   &Mortal::RawAccessFailure { ref file_name, ref cause } => {
                       format!("Can't access a raw file: {}. Reason: {}!", file_name, cause)
                   }
                   &Mortal::DefinitionDecodeFailure { ref ini_name, ref cause } => {
                       format!("Failed to decode definition from ini: {}. Reason: {}!",
                               ini_name,
                               cause)
                   }
                   &Mortal::DefinitionLoadFailure { ref ini_name, ref cause } => {
                       format!("Failed to load definition from ini: {}. Reason: {}!",
                               ini_name,
                               cause)
                   }

                   &Mortal::CheckURL { ref service, ref url, ref cause } => {
                       format!("Failed URL check for: {} of: {}. Reason: {}!",
                               url,
                               service,
                               cause)
                   }
                   &Mortal::CheckURLFail { ref service, ref cause } => {
                   ...
                   &Mortal::CheckPidfileUnaccessible { ref service, ref cause } => {
                       format!("Cannot access pid file for: {}. Reason: {}!",
                               service,
                               cause)
                   }
                   &Mortal::CheckUnixSocket { ref service, ref cause } => {
                       format!("Couldn't connect through UNIX socket: {} of: {}. Reason: {}!",
                               service.unix_socket(),
                               service,
                               cause)
                   }
                   &Mortal::CheckUnixSocketMissing { ref service, ref cause } => {
                       format!("Missing expected UNIX socket: {} of: {}. Reason: {}!",
                               service.unix_socket(),
                               service,
                               cause)
                   }
                   ...
                   &Mortal::ServiceStartFailure { ref service, ref cause } => {
                       format!("Failed to launch commands: {} for {}! Reason: {}",
                               service.clone().start.unwrap_or("#no-commands".to_string()),
                               service,
                               cause)
                   }
                   ...
                   &Mortal::NotificationConfigFailure { ref service, ref cause } => {
                       format!("Service: {} has notification config problem: {}",
                               service,
                               cause)
                   }
