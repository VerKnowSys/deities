...
impl Perun for Service {
    fn try_urls(&self) -> Result<Mortal, Mortal> {
        for url in self.urls() {
            ...
            easy.dns_cache_timeout(Duration::from_millis(self.clone().checks_url_timeout()))
                .unwrap();
            ...
            Ok(data) => {
                match String::from_utf8(data.stdout) {
                    Ok(parsed) => {
                        ...
                        debug!("Free disk space: {} MiB. (inodes: Skipped for Linux)",
            ...
            Ok(data) => {
                match String::from_utf8(data.stdout) {
                    Ok(parsed) => {
                        ...
                        debug!("Free disk space: {} GiB. Free inodes: {}",
    ...
    fn checks_for(&self) -> Result<Mortal, Mortal> {
        ...
        match self.unix_socket().as_ref() {
            ...
            _ => {
                match self.try_unix_socket() {
                    Ok(_) => {
                        checks_performed.fetch_add(1, Ordering::SeqCst);
                        debug!("UNIX socket check passed for: {}, with unix_socket: {}",
                               self.styled(),
                               self.unix_socket())
                    }
        ...
        match self.pid_file().as_ref() {
            ...
            _ => {
                match self.try_pid_file() {
                    Ok(_) => {
                        checks_performed.fetch_add(1, Ordering::SeqCst);
                        debug!("PID check passed for: {}, with pid_file: {}",
                               self.styled(),
                               self.pid_file())
                    }
                    Err(err) => return Err(err),
        ...
        if self.urls().len() > 0 {
            match self.try_urls() {
                Ok(_) => {
                    checks_performed.fetch_add(1, Ordering::SeqCst);
                    debug!("URLs check passed for: {}, with urls: {:?}",
                           self.styled(),
                           self.urls())
                }
                Err(err) => return Err(err),
        ...
        trace!("performed {} checks for: {}",
               checks_performed.clone().load(Ordering::SeqCst),
               self.styled());
        match checks_performed.clone().load(Ordering::SeqCst) {
            ...
            _ => {
                Ok(OkAllChecks {
                    service: self.clone(),
                    amount: checks_performed.clone().load(Ordering::SeqCst) as i32,
                })
            }
        }
    }
